...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $rsAgents = $entity::GetList([
             ...
             */
            'filter' => array("!UF_ACTIVITY"=>false),
            'count_total' => true,
    ...
    public function clickStarAction($agentID)
    {
         ...
         */
        $userOption = CUserOptions::GetOption('mcart_agent', 'agents_star');

        if ($userOption) {
            if (!is_array($userOption)) $userOption = array();
            if (in_array($agentID,$userOption)) unset($userOption[array_search($agentID,$userOption)]);
            else array_push($userOption,$agentID);
            $value = $userOption;
        } else {
            array_push($value, $agentID);
        }
        CUserOptions::SetOption('mcart_agent','agents_star',$value);
